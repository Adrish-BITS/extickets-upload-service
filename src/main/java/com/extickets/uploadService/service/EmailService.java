...
public class EmailService {
    ...
    @Autowired
    private RestTemplate restTemplate; // ✅ Injected bean instead of new RestTemplate()
    ...
    public void sendEmail(Ticket ticket, String eventImageUploadUrl) {
        ...
        String userHtmlBody = UserEmailTemplates.TICKET_UPLOADED.getTemplate()
                .replace("${eventName}", ticket.getEventName())
                .replace("${eventImageUploadUrl}", eventImageUploadUrl)
                .replace("${venue}", ticket.getVenue())
                .replace("${eventDateTime}", ticket.getEventDateTime().format(formatter))
                .replace("${price}", ticket.getPrice().toString())
                .replace("${status}", "Review");

        sendRequestToEmailService("2023tm93518@wilp.bits-pilani.ac.in",
                "[ExTickets] Tickets Uploaded successfully", userHtmlBody);
        ...
        sendRequestToEmailService("backuponeplus345@gmail.com",
                "[ExTickets] New Ticket Uploaded - Review Required", adminHtmlBody);
    }
